...
#define PORT 17001 // Port for Spdf server
#define BUFFER_SIZE 1024

// Function to create necessary directories for the Spdf server
void create_directory() {
    mkdir("spdf", 0755);
}

void manage_server(int new_socket) {
    ...
    printf("Spdf Received command: %s\n", path);
    ...
    if (strncmp(command, "rmfile", 6) == 0) {
        ...
        if (remove(filename) == 0) {
            ...
            char *response = "File deleted successfully by pdfserver.\n";
            send(new_socket, response, strlen(response), 0);
        } else {
            perror("Failed to delete file");
            char *response = "Failed to delete file on pdfserver.\n";
            send(new_socket, response, strlen(response), 0);
        }
    } 
    //Handle UFILE command recieved from Smain
        ...
        snprintf(full_path, sizeof(full_path), "spdf/%s/%s", destination_path + 7, filename);

        // Create directories if needed
        char mkdir_command[BUFFER_SIZE];
        snprintf(mkdir_command, sizeof(mkdir_command), "mkdir -p spdf/%s", destination_path + 7);
...
int main() {
    ...
    printf("Spdf is listening on port %d...\n", PORT);
